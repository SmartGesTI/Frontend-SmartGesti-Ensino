...
export function HelpPanel() {
  ...
  // Calcular largura baseado na prop width
  const getWidthClass = () => {
    if (!content?.width || content.width === 'default') {
      return 'w-[320px] sm:w-[380px]'
    }
    if (content.width === 'large') {
      return 'w-[640px] sm:w-[760px]'
    }
  ...
  const widthStyle = typeof content?.width === 'number' 
  ...
  return (
    ...
    >
      {content && (
        <>
          {/* Header Fixo - apenas para conteúdo help */}
          ...
          {/* Header para conteúdo customizado - com botão fechar */}
          {content.variant === 'custom' && (
            <div className="flex-shrink-0 px-6 py-4 border-b border-gray-200 dark:border-gray-700 flex items-center justify-end">
              <button
                onClick={handleClose}
                className="rounded-lg p-1.5 opacity-70 transition-all hover:opacity-100 hover:bg-gray-100 dark:hover:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-purple-500/20"
              >
                <X className="h-5 w-5 text-gray-500 dark:text-gray-400" />
              </button>
            </div>
          ...
          <div className={cn(
            ...
            content.variant === 'custom' ? '' : 'px-6 py-4 overflow-y-auto'
          )}>
            {content.customContent ? (
              // Renderizar conteúdo customizado (já tem seu próprio padding e scroll)
              content.customContent
            ) : content.items ? (
              // Renderizar lista de HelpItems (comportamento original)
          ...
          {/* Footer - apenas para conteúdo help */}
          {content.variant !== 'custom' && content.items && (
